...
function VidyoClientInitialize($) {
	VCUtils.version = "4.1.0.8";
    console.log("ABHI INIT VIDEO CLIENT")
	...
	if(isIE) {
		...
		VCUtils.webRTCAvailable = navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.msGetUserMedia;
	...
	if (VCUtils.params.plugin != "false" && VCUtils.windowParams.plugin != "false" && VCUtils.pluginAvailable) {
		...
		VCUtils.vcOnLoadComplete = false;
		...
		es6.onload = function(){
			vcd.onload = function(){
				vct.onload = function(){
					...
					VC = new VidyoClient("VidyoClientPlugIn", VCUtils.onloadCallback);
				}
		...
		setTimeout($.proxy(VidyoClientOnLoadError, this), 4000);
		
	} else if (VCUtils.params.webrtc != "false" && VCUtils.windowParams.webrtc != "false" && VCUtils.webRTCAvailable) {
		/* load multiple js files at the same time */
        console.log("ABHI IN WRTC" + VCUtils.jsPath);
        VCUtils.jsPath = "https://static.vidyo.io/4.1.0.8/";
		$.when(
                ...
                $.getScript("es6-promise.auto.min.js"),
				$.getScript("VidyoClientDispatcher.js"),
				$.getScript("VidyoClientTransportWebRTC.js"),
				...
				if (VCUtils.onloadCallback) {
					VCUtils.onloadCallback({state: "FAILED", description: "Unable to load resources", type: "LOAD"});
				}
		});
	} else {
		/* load plugin download helper after 1 sec to allow the rest of the page to load */
		setTimeout($.proxy(VidyoClientNotAvailable, this), 1000);
	}
}

function VidyoClientOnLoadError(){
...
function VidyoClientNotAvailable() {
	...
	if (!isMac && !inWin) {
		/* Mobile */
		downloadType = "MOBILE";
	} else if (isSafari || isFirefox || (isIE && !isEdge)) {
		/* Supports Plugins */
		downloadType = "PLUGIN";
	} else {
		/* Supports App */
	...
	if (VCUtils.onloadCallback({state: "NOTAVAILABLE", description: "Not available", type: "LOAD", downloadType: downloadType, downloadPathApp: downloadPathApp, downloadPathPlugIn: downloadPathPlugIn})) {
		/* If returned true, try to initialize again in case the plugin was installed. */
